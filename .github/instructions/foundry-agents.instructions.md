...
- **Connected-agent topology:** one `fraud-triage-agent` orchestrator delegating to
  `fraud-investigation-agent`, `fraud-aml-agent`, `fraud-claims-agent`. Publish each agent
  separately; each gets its own Agent Identity — reconfigure its Azure permissions after publish.
