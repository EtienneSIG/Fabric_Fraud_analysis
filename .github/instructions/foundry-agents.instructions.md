...
- **Web IQ (regulatory):** the Web IQ experience is powered by Foundry's native **Web Search** tool;
  it requires no separate API key or MCP connection. Restrict every search to the official-domain
  allow-list with `site:` operators and validate citations against it; NEVER put case PII in a web
  query (generic legal terms only). The app surfaces citations returned by the Foundry response.
