...
import type { FraudIntelSchema } from '../../rayfin/data/schema';
...
let client: RayfinClient<FraudIntelSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<FraudIntelSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<FraudIntelSchema>({
...
export function getRayfinClient(): RayfinClient<FraudIntelSchema> {
  if (!client) {
    throw new Error(
